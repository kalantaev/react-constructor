...
export function createDoor(parent, price, name, el) {
    let door;
    if (parent) {
        let x = parent.getX() + ((parent.getLength() / 4) - (doorLength / 2));
        let y = parent.getY() - getKarkasDepth() + parent.getScaledWidth();
        door = createShape(x, y, doorLength, defPartitionWidth, colorBlack);
        door.direction = DIRECTION.RIGHT;
        door.opening = OPENING_TYPE.OUT;
        door.position = POSITION.BOTTOM;
        door.shift = 50;
        door.parentId = parent.getId();
        door.position = parent.isHorizontally() ? POSITION.BOTTOM : POSITION.RIGHT;
        door.price = price;
        door.printName = name;
        door.hint = name;
        door.height = el.height;
        door.image3D = el.image3D;
        door.settingId = el.id
    } else {
        door = createShape();
    }
    door.name = "Door";
